...
const Body = () => {
    ...
    return (allRestaurants.length === 0) ? <Shimmer/> : (
        ...
        <div className="flex flex-wrap ">
            {(filteredRestaurants?.length === 0) ? <h1>No Restaurants found matching your filter</h1> :
            filteredRestaurants.map((restaurant)=> {
                return (
                <Link className="link" to={`/restaurant/${restaurant.data.id}`} key={restaurant.data.id}>
